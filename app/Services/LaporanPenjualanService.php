    ...
    public function produkTerlarisHariIni(int $limit = 5)
{
    return DB::table('item_penjualan')
        ...
        ->select(
            'produk.nama',
            'produk.stok',
            DB::raw('SUM(item_penjualan.kuantitas) as unit_terjual')
        )
        ->orderByDesc('unit_terjual')
        ->limit($limit)
        ->get();
}

    public function jumlahProdukTerlarisHariIni(): int
    {
        return DB::table('item_penjualan')
            ->join('penjualan', 'penjualan.id', '=', 'item_penjualan.penjualan_id')
            ->whereDate('penjualan.created_at', Carbon::today())
            ->where('penjualan.status', 'COMPLETED')
            ->distinct('item_penjualan.produk_id')
            ->count('item_penjualan.produk_id');
    }
}